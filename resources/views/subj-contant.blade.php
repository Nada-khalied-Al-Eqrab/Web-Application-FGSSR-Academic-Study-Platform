...
@section('contant')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="card">
                    <img class="card-img-top" src="{{ $course->img_url }}" alt="Card image cap" style="max-height: 450px">
                </div>
            </div>
            <!-- End Row -->
            <!-- ============================================================== -->
                ...
                <div class="card">
                    <div class="card-body">
                        <span class="btn btn-dark float-right"><i class="mdi mdi-account-multiple"></i>
                            {{ $studentsCount }}</span>
                        <span class="btn btn-dark float-left">{{ $course->language }}</span>
                        <h4 class="card-title" style="text-align: center;"> <strong>{{ $course->name_ar }}</strong></h4>
                        <h4 class="card-title" style="text-align: center;"> <strong>{{ $course->code }} -
                                {{ $course->name_en }} </strong>
                        </h4>
                        ...
                        <div class="tab-content mt-4" id="registrationTabContent">
                            <div class="tab-pane fade show active" id="info" role="tabpanel"
                                ...
                                <section style="text-align: center;">
                                    ...
                                    @endphp
                                    @if ($hasMaterial || $hasLectures || $hasExams)
                                        @if ($hasMaterial)
                                            <section style="text-align: right;">
                                                @include('partial.materials', ['material' => $material])
                                            </section>
                                        @endif
                                        @if ($hasLectures)
                                            <section style="text-align: center;">
                                                @include('partial.lecture', ['lectures' => $lectures])
                                            </section>
                                        @endif
                                        @if ($hasExams)
                                            <section style="text-align: center;">
                                                @include('partial.exam', ['exams' => $exams])
                                            </section>
                                        @endif
                                    @else
                    ...
                    async function fetchMessages() {
                        try {
                            ...
                            messages.forEach(msg => {
                                ...
                                li.className = (msg.user_id == {{ auth()->id() }}) ? 'odd chat-item' :
                                    'chat-item';
                                li.innerHTML = `
                                    ${msg.user_id != {{ auth()->id() }} ? `<div class="chat-img">
                                                    <img src="../assets/images/users/default/default.png" alt="User">
                                                </div>` : ''}
                                    <div class="chat-content">
                                        ${msg.user_id != {{ auth()->id() }} ? `<h6 class="font-medium">${msg.user_name}</h6>` : ''}
                                        <div class="box ${msg.user_id == {{ auth()->id() }} ? 'bg-light-inverse' : 'bg-light-info'}">
                                            ${msg.message}
                                        </div>
                                    </div>
